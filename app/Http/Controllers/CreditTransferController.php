...
class CreditTransferController extends BasePageController
{
    public function index()
    {
        $globals = $this->loadGlobals();
        $pageData = json_decode(Storage::get('content/pages/credit-transfer.json'), true) ?? [];
        ...
        if (!empty($globals['navigation']['header']['menu'])) $header['menu'] = $globals['navigation']['header']['menu'];
        if (!empty($globals['navigation']['header']['category'])) $header['category'] = $globals['navigation']['header']['category'];

        $globalHeaderText = $globals['text']['header'] ?? [];
        $globalContact = $globals['text']['contact'] ?? [];

        if (!empty($globalHeaderText['top'])) $header['top'] = $globalHeaderText['top'];
        ...
        if (!empty($globalHeaderText['side'])) $header['side'] = $globalHeaderText['side'];
        ...
        if (!empty($globalHeaderText['buttons'])) $header['buttons'] = $globalHeaderText['buttons'];

        $footer = $homeV1['footer'] ?? [];
        if (!empty($globals['navigation']['footer'])) {
            foreach (['navigate', 'courses', 'bottom'] as $key) {
                if (!empty($globals['navigation']['footer'][$key])) $footer[$key] = $globals['navigation']['footer'][$key];
            }
        }
        ...
        return view('pages.credit-transfer', [
            'fonts'  => $globals['fonts'],
